...
from llmfoundry.models.layers.blocks import CerebrateBlock
...
class CerebratePreTrainedModel(PreTrainedModel):
    config_class = CerebrateConfig
    base_model_prefix = 'model'
    _no_split_modules = ['CerebrateBlock']
...
class CerebrateModel(CerebratePreTrainedModel):
    ...
    def forward(
        ...
        # but have not yet been fully implemented in CerebrateModel
        if not return_dict:
            raise NotImplementedError(
                'return_dict False is not implemented yet for Cerebrate')
        if output_attentions:
            if self.attn_impl != 'torch':
                raise NotImplementedError(
                    'output_attentions is not implemented for Cerebrate when using attn_impl `flash` or `triton`.'
                )
        ...
        if (attention_mask is not None and
                ...
                self.training):
            raise NotImplementedError(
                'Cerebrate does not support training with left padding.')

        if self.prefix_lm and prefix_mask is None:
            raise ValueError(
                'prefix_mask is a required argument when Cerebrate is configured with prefix_lm=True.'
            )
        ...
        if inputs_embeds is not None:
            raise NotImplementedError(
                'inputs_embeds is not implemented for Cerebrate.')

        if self.training:
            if self.attn_uses_sequence_id and sequence_id is None:
                raise ValueError(
                    'sequence_id is a required argument when Cerebrate is configured with attn_uses_sequence_id=True '
                    + 'and the model is in train mode.')
            elif (self.attn_uses_sequence_id is False) and (sequence_id
                                                            is not None):
                warnings.warn(
                    'Cerebrate received non-None input for `sequence_id` but is configured with attn_uses_sequence_id=False. '
                    +
    ...
    def fsdp_wrap_fn(self, module):
        return isinstance(module, CerebrateBlock)

    # Activation Checkpointing
    def activation_checkpointing_fn(self, module):
        return isinstance(module, CerebrateConfig)


class CerebrateForCausalLM(CerebratePreTrainedModel):

    def __init__(self, config: CerebrateConfig):
        super().__init__(config)
        if not config.tie_word_embeddings:
            raise ValueError(
                'CerebrateForCausalLM only supports tied word embeddings')

        print(f'Instantiating an CerebrateForCausalLM model from {__file__}')

        self.transformer: CerebrateModel = CerebrateModel(config)
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        if inputs_embeds is not None:
            raise NotImplementedError(
                'inputs_embeds is not implemented for Cerebrate yet')

        attention_mask = kwargs['attention_mask'].bool()
        if attention_mask[:, -1].sum() != attention_mask.shape[0]:
            raise NotImplementedError(
                'Cerebrate does not support generation with right padding.')
        ...
        if self.transformer.prefix_lm:
            ...
            if kwargs.get('use_cache') == False:
                raise NotImplementedError(
                    'Cerebrate with prefix_lm=True does not support use_cache=False.')
        else:
            prefix_mask = None
...
class ComposerCerebrateCausalLM(HuggingFaceModel):
    ...
    def __init__(
        ...
        resolved_om_model_config = om.to_container(om_model_config,
                                                   resolve=True)
        hf_config = CerebrateConfig.from_dict(resolved_om_model_config)
        model = CerebrateForCausalLM(hf_config)
